          ...
          {projects.map((project) => (
            ...
            >
              <div className="w-full bg-black p-1 border border-[#00ff00] overflow-hidden relative mb-3">
                ...
                <div className="overflow-x-auto -mx-1" style={{ maxWidth: '100%' }}>
                  <pre className="font-mono text-[3px] sm:text-[4px] leading-[3px] sm:leading-[4px] text-[#00ff00]/90 select-none" style={{ textShadow: '0 0 1px #00ff00', maxWidth: '100%' }}>
                    {project.chafa.map((line, i) => (
                      <div key={i} style={{ wordBreak: 'break-all', overflowWrap: 'break-word' }}>{line}</div>
                    ))}
                  </pre>
                </div>
              </div>
